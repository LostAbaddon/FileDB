...
class FileDB extends EventEmitter {
	...
	constructor (filepath, option) {
		...
		var opt = {};
		if (!!option) {
			let o = FileDB.copyOption(option);
			if (o !== null) opt = o;
		}
	...
	set (...args) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'set', res, this);
	...
	get (...args) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'get', res);
	...
	append (value) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'append', res, this);
	...
	prepend (value) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'prepend', res, this);
	...
	insertBefore (index, value) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'insertBefore', res, this);
	...
	insertAfter (index, value) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'insertAfter', res, this);
	...
	remove (...args) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'remove', res, this);
	...
	findByIndex (index, key) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'findByIndex', res);
	...
	filter (cb, env) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'filter', res);
	...
	forEach (cb, env) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'forEach', res);
	...
	flush (immediate=false) {
		return new Promise(res => {
			...
			var tid = FileDB.regCallback(this.worker, 'flush', res, this);
	...
	static copyOption (option) {
		...
		if (option instanceof Array) return option.map(ele => FileDB.copyOption(ele));
		...
		for (let key in option) {
			let value = FileDB.copyOption(option[key]);
	...
	static regCallback (worker, event, res, host) {
		var tid = 'task::' + FileDB.newID();
